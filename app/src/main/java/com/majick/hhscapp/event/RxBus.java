...
public class RxBus {
     ...
     * @return
     */
    private <T> Disposable doSubscribe(Class<T> eventType, Consumer<T> action, Consumer<Throwable> error) {
     ...
     * 保存订阅后的disposable
     */
    private void addDisposable(Object object, Disposable disposable) {
    ...
    public <T> void register(Object subscribe, Class<T> event, Consumer<T> action) {
        Disposable disposable = doSubscribe(event, action, throwable -> {
